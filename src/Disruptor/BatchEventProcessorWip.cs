...
namespace Disruptor
{
    ...
    public sealed class BatchEventProcessorWip<T> : IEventProcessor where T : class
    {
        ...
        private readonly MethodInfo _runMethodInfo;
        private readonly object[] _runMethodParameters;
        ...
        public BatchEventProcessorWip(IDataProvider<T> dataProvider, ISequenceBarrier sequenceBarrier, IEventHandler<T> eventHandler)
        {
            ...
            var sequenceBarrierProxy = StructProxy.CreateSequenceBarrier(_sequenceBarrier);
            var batchStartAwareProxy = StructProxy.CreateBatchStartAware(_eventHandler);
            var dataProviderProxy = StructProxy.CreateDataProvider(_dataProvider);
            var eventHandlerProxy = StructProxy.CreateEventHandler(_eventHandler);

            var runGenericMethod = GetType().GetMethod(nameof(RunImpl), BindingFlags.Instance | BindingFlags.NonPublic);
            _runMethodInfo = runGenericMethod.MakeGenericMethod(sequenceBarrierProxy.GetType(), batchStartAwareProxy.GetType(), dataProviderProxy.GetType(), eventHandlerProxy.GetType());
            _runMethodParameters = new object[] { sequenceBarrierProxy, batchStartAwareProxy, dataProviderProxy, eventHandlerProxy };
        }
        ...
        public void Run()
        {
            _runMethodInfo.Invoke(this, _runMethodParameters);
        }

        private void RunImpl<TSequenceBarrier, TBatchStartAware, TDataProvider, TEventHandler>(TSequenceBarrier sequenceBarrier, TBatchStartAware batchStartAware, TDataProvider dataProvider, TEventHandler eventHandler)
